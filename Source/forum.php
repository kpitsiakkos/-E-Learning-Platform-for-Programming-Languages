...
if (isset($_POST['submit'])) {
    ...
    $reply_comment_id = isset($_POST['reply_comment_id']) ? intval($_POST['reply_comment_id']) : 0;
    $comment_date = date('Y-m-d H:i:s');

    if (!empty($user_name) && !empty($comment)) {
        try {
            $insert_comment = $dbConn->prepare("INSERT INTO chat (user_name, comment, comment_date, reply_comment_id) VALUES (?,?,?,?)");
            $insert_comment->execute([$user_name, $comment, $comment_date, $reply_comment_id]);

            // Redirect to the same page or another one
            header("Location: forum.php"); // Adjust the target URL as needed
            exit(); // Don't forget to call exit after header()
        } catch (PDOException $e) {
            echo "Error: " . $e->getMessage();
        }
    } else {
        echo '<div class="error-message">Username and comment must not be empty!</div>';
    }
}
?>
...
<!-- THIS SECTION IS WHERE USERS CAN EXCHANGE THEIR OPINION ABOUT DIFFERENT SUBJECTS -->
...
<div class="discussion">
    ...
    <?php
    try {
        $select_comments = $dbConn->prepare('SELECT * FROM chat WHERE reply_comment_id = 0');
